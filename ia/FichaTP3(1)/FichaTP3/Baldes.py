...
class Balde():
    ...
    def cria_grafo(self):
        # Implementação da criação do grafo a partir do estado inicial e das ações possíveis
        for estado in range(0, self.balde1 + 1):
            for estado2 in range(0, self.balde2 + 1):
                self.g.adiciona_arco(f'({estado},{estado2})', self.enche1(f'({estado},{estado2})'))
                self.g.adiciona_arco(f'({estado},{estado2})', self.enche2(f'({estado},{estado2})'))
                self.g.adiciona_arco(f'({estado},{estado2})', self.despeja12(f'({estado},{estado2})'))
                self.g.adiciona_arco(f'({estado},{estado2})', self.despeja21(f'({estado},{estado2})'))
                self.g.adiciona_arco(f'({estado},{estado2})', self.esvazia1(f'({estado},{estado2})'))
                self.g.adiciona_arco(f'({estado},{estado2})', self.esvazia2(f'({estado},{estado2})'))


    # Dado um estado, expande para outros mediante as açoes possiveis
    def expande(self,estado):
        return [self.enche1(estado), self.enche2(estado), self.despeja12(estado), self.despeja21(estado),self.esvazia1(estado), self.esvazia2(estado)]
